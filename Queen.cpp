...
void Queen::move(sf::RenderWindow& window, bool kingSelected, bool kingReleased,
                sf::Sprite& queens, const sf::Vector2i& mouse, int positionX, int positionY)
{
    ...
    for (int i = 0; i < 1; i++)
    {
        if (kingSelected && queens.getGlobalBounds().contains(window.mapPixelToCoords(mouse)))
        {
